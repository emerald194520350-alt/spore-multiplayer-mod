...
The project is currently **Beta 12** (protocol 7), an experimental prerelease. This update addresses player 2 showing a carnivore mouth while retaining the original avatar's gameplay abilities: the guest now adopts the shared native body and campaign creature key once its model is loaded and baked. Native body replacement preserves the avatar, health and progression. Earlier crash guards, shared evolution, appearance, DNA, parts, quests, movement, eating animations, yellow teammate arrow, History timeline and owner-only saves remain included. Update both game DLLs; the protocol-7 server is compatible with Beta 9–11. Automated checks pass; meat pickup in two game windows and the earlier editor/save scenarios still require interactive verification. See [release notes](RELEASE_NOTES.md) for scope, validation and installation.
