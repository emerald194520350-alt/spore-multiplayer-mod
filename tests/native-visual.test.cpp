...
int main(int argc, char** argv)
{
    try
    {
        ...
        gRole="host"; gSnapshot=CoopNet::Snapshot{};
        gSnapshot.connected=gSnapshot.inviteAccepted=true;
        gSnapshot.worldGeneration=9;gSnapshot.inviteFrom="host";
        const auto plant=R"({"type":"dietEvent","revision":10,"role":"guest","worldGeneration":9,"sequence":1,"eventID":2666926355})";
        HandleMessage(plant); HandleMessage(plant);
        Check(CoopNet::TakeDietEvents(9).empty(),"History waits until the corresponding food revision has been applied");
        auto diet=CoopNet::TakeDietEvents(10);
        Check(diet.size()==1 && diet[0]==0x9ef61113u,"Remote plant history event is consumed once");
        Check(CoopNet::TakeDietEvents(10).empty(),"Applied diet events cannot replay on the next frame");
        HandleMessage(R"({"type":"dietEvent","revision":10,"role":"guest","worldGeneration":8,"sequence":2,"eventID":2893111733})");
        HandleMessage(R"({"type":"dietEvent","revision":10,"role":"guest","worldGeneration":9,"sequence":2,"eventID":123})");
        Check(CoopNet::TakeDietEvents(10).empty(),"Wrong world and unrelated history event are rejected");
        HandleMessage(R"({"type":"dietEvent","revision":10,"role":"guest","worldGeneration":9,"sequence":2,"eventID":2893111733})");
        diet=CoopNet::TakeDietEvents(10);
        Check(diet.size()==1 && diet[0]==0xac7161b5u,"Remote meat history type is preserved");
        MarkDisconnected();
        gRole="guest";
